...
// Each lifecycle run uses its own socket ("wonka-{runID}") to prevent
...
func NewTmuxClient(runID string) *TmuxClient {
	return &TmuxClient{Socket: "wonka-" + runID, runID: runID}
}
...
// (BVV Appendix A — exit-code capture mechanism).
//
// Uses shell export instead of tmux -e (available only in tmux 3.2+) for
// broader compatibility. The command runs as a child process (not exec) so
// that bash can write the exit code to {logPath}.exitcode after the command
// exits. The tmux session ends when bash exits.
//
// When textFilter is non-empty, the command output is piped through tee (to
// capture raw output in logPath) and then through jq (to extract filtered
// text into a .txt file). This enables stream-json logging: the .stdout file
// gets full JSONL, the .txt file gets human-readable assistant text. The exit
// code uses PIPESTATUS[0] to capture the agent's exit code (not jq's). If jq
// is unavailable, the .txt file is empty but .stdout and .exitcode are
// unaffected.
//
// Returns ErrInvalidEnvKey wrapped with context if any env key is not a valid
// POSIX identifier.
