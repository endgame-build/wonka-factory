...
var (
	...
	// Returned by BDCLIStore when a `bd` invocation times out, is cancelled by
	// the orchestrator's context, or exits with a transport-class signal (exec
	...
	//
	// Cancellation note: when wrapped around a context.Canceled or
	// context.DeadlineExceeded, the underlying context error is preserved via
	// multi-%w so classifyEngineError's signal-interrupt clause (which fires
	// before the ErrStoreUnavailable clause) still routes signal-driven
	// shutdown to exitSignalInterrupt.
