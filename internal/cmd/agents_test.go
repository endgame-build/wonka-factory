...
// A failure here means an agent launch would fail at runtime with an opaque
// error; tripping here early is much cheaper.
...
func TestAgentInstructionFilesForbiddenPatterns(t *testing.T) {
	...
	roleRequired := map[string][]string{
		"OOMPA.md":   {"exit 3"},    // handoff allowed (spec §6.2)
		"LOOMPA.md":  {"exit 3"},    // handoff allowed (spec §6.2)
		"CHARLIE.md": {"bd create"}, // planner writes to beads
	}
