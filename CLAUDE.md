...
- **Store factory** (`NewStore(kind, dir) → (Store, LedgerKind, error)`): Empty kind defaults to `"beads"` and falls back to `"fs"` when Beads/Dolt is unavailable. Explicit `LedgerBeads` is strict — no silent FS fallback, so a misconfigured operator does not write FS-store JSON into a directory bd manages. Returns the actual backend kind so callers can detect fallback when permitted. CLI flag `--ledger` always sets the kind explicitly.
- **Ledger location** (`ResolveLedgerDir(repoPath, runDir, kind, override) string`): `LedgerBeads → <repoPath>/.beads/` (shared with `bd`; the BVV-DSN-04 contract — wonka and Charlie open the same store). `LedgerFS → <runDir>/ledger/` (per-run dev convenience). Empty kind also routes to `<runDir>/ledger/` for tests using `DefaultEngineConfig`. The `override` parameter is non-empty only via `Engine.SetTestLedgerDir` (test seam, not exposed on the CLI).
- **Auto-init for `--ledger beads`**: When `<repo>/.beads/` is missing, `Engine.init()` runs `bd init --stealth --non-interactive --quiet` so operators do not need a separate bootstrap step. `BuildEngineConfig` fail-fasts with `ErrBeadsCLIMissing` when `bd` is not on PATH (the beads SDK has no programmatic Init). Auto-init prints a stderr warning naming the path so operators notice that wonka mutated the working tree before committing — the warning is not emitted to `events.jsonl`.
